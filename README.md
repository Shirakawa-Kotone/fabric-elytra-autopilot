...
"Too slow" is not a guessed number. It is computed from the movement model as **the slowest speed at the bottom of a dive from which a pull-up can still win back the height a climb cycle trades away**. A climb cycle gives away about 74 blocks in its dive (the figure the strategy mode is built around), and the model puts the speed that wins those 74 blocks back at **2.2 blocks/tick, 44 m/s** - just under the speed the dive cycle is built to reach, and comfortably above cruise, which is why a cruise-speed pull-up does not buy height. Above that speed plus **Climb Speed Margin** (0 blocks/tick by default) avoidance climbs; below it, it dives. The number follows the player's gravity - a slow falling potion drops it to about 19 m/s - and is worked out once at startup and logged.
